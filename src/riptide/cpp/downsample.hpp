...
*/
void complex_downsample(const std::complex<float>* __restrict__ in, size_t size, double f, std::complex<float>* __restrict__ out)
    {
    check_downsampling_factor(size, f);
    ...
    for (size_t k = 0; k < n; ++k)
        ...
        std::complex<float> acc = wmin * in[imin];
        for (size_t i = imin + 1; i < imax; ++i)
            acc += in[i];
        acc += wmax * in[imax];
