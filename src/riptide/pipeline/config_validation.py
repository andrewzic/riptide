...
SEARCH_RANGE_SCHEMA = Schema(
    {
        ...
        "ffa_search": {
            "period_min": And(
                Use(float), strictly_positive, error="period_min must be a number > 0"
            ),
            "period_max": And(
                Use(float), strictly_positive, error="period_max must be a number > 0"
            ),
            "bins_min": And(
                int, strictly_positive, error="bins_min must be an int > 0"
            ),
            "bins_max": And(
                int, strictly_positive, error="bins_max must be an int > 0"
            ),
            Optional("fpmin"): And(
                int, strictly_positive, error="fpmin must be an int > 0"
            ),
            Optional("wtsp"): And(
            ...
            Optional("ducy_max"): And(
                float,
                lambda x: 0 < x < 1,
                error="ducy_max must be strictly between 0 and 1",
            ),
        },
        "find_peaks": {
            Optional("smin"): And(
            ...
            Optional("clrad"): Or(
                And(Use(float), strictly_positive),
                None,
                error="clrad must be a number > 0",
            ),
        },
        "candidates": {
            "bins": And(
                int, strictly_positive, error="candidates.bins must be an int > 0"
            ),
            "subints": And(
                int, strictly_positive, error="candidates.subints must be an int > 0"
            ),
        },
    }
)
...
PIPELINE_CONFIG_SCHEMA = Schema(
    {
        ...
        "dmselect": {
            ...
            "max": Or(
                Use(float), None, error="Maximum DM must be a number or null/blank"
            ),
            "dmsinb_max": Or(
                strictly_positive,
                None,
        ...
        "dereddening": {
            "rmed_width": Schema(
                strictly_positive, error="rmed_width must be a number > 0"
            ),
            "rmed_minpts": Schema(
                strictly_positive, error="rmed_minpts must be a number > 0"
            ),
        },
        "ranges": [SEARCH_RANGE_SCHEMA],
        "clustering": {
            "radius": Schema(
                strictly_positive, error="clustering radius must be a number > 0"
            ),
        },
        "harmonic_flagging": {
            "denom_max": And(
                int, strictly_positive, error="denom_max must be an int > 0"
            ),
            "phase_distance_max": And(
                Use(float),
                strictly_positive,
                error="phase_distance_max must be a number > 0",
            ),
            "dm_distance_max": And(
                Use(float),
                strictly_positive,
                error="dm_distance_max must be a number > 0",
            ),
            "snr_distance_max": And(
                Use(float),
                strictly_positive,
                error="snr_distance_max must be a number > 0",
            ),
        },
        "candidate_filters": {
            "dm_min": Or(
                Use(float), None, error="Candidate dm_min must be a float or null/blank"
            ),
            "snr_min": Or(
                Use(float),
                None,
                error="Candidate snr_min must be a float or null/blank",
            ),
            "remove_harmonics": Or(
