...
from riptide import __version__, ffa_search, vis_ffa_search, find_peaks, Candidate
...
from riptide.pipeline.worker_pool import WorkerPool, VisWorkerPool
...
class VisPipeline(object):
    """
    Top-level class that runs a multiple DM trial search on visibilities.
    ...
    @timing
    def vis_search_basep(self, real_files, imag_files):
        """
        vis-search the selected file
        """
        log.info("Running search")
        all_candidates = []
        for real_file, imag_file in zip(real_files, imag_files):
            candidates = self.worker_pool.process_uvcells_basep(real_file, imag_file)
            all_candidates.append(candidates)
        return all_candidates
