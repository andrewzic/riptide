...
for img, first_img, first_grid, period in zip(d[0]["images"], d[0]["first_images"], d[0]["first_grids"], d[0]["periods"]):
    ...
    axs[1].imshow(first_img, origin='lower', interpolation='none', aspect='auto', vmin=-0.003, vmax=0.003)
    axs[2].imshow(img, origin='lower', interpolation='none', aspect='auto', vmin=0.2, vmax=3.0)
